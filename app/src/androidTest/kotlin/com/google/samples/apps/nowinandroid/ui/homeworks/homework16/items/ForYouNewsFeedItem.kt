...
class ForYouNewsFeedItem(
    semanticNode: SemanticsNode,
    semanticsProvider: SemanticsNodeInteractionsProvider?
) : KLazyListItemNode<ForYouNewsFeedItem>(semanticNode, semanticsProvider) {

    val title by lazy {
        child<KNode> {
            hasTestTag(TestingTag.NEWS_TITLE)
        }
    }

    val image by lazy {
        child<KNode> {
            hasTestTag(TestingTag.NEWS_IMAGE)
        }
    }
}